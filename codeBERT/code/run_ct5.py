...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          RobertaConfig, RobertaModel, RobertaTokenizer,T5ForConditionalGeneration, T5Config)
      ...
      #                          t=t[:t.index(0)]
       #                     text = tokenizer.decode(t,clean_up_tokenization_spaces=False)
        #                    p.append(text)
                ...
                logger.info('dev set: '
                            'bleu = %.2f | dev_bleu = %.2f | rouge_l = %.2f | meteor = %.2f | ' %
                            (bleu4, dev_bleu, rouge_l, meteor) +
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            for batch in eval_dataloader:
                ...
                source_ids,source_mask= batch                  
                with torch.no_grad():
                    preds = model.generate(source_ids,attention_mask=source_mask,use_cache=True,num_beams=args.beam_size,early_stopping=True, max_length=args.max_target_length)
                    top_preds = list(preds.cpu().numpy())
                    pred_ids.extend(top_preds)
            p = [tokenizer.decode(id, skip_special_tokens=True, clean_up_tokenization_spaces=False) for id in pred_ids]
